/*
 * io.socket.io-java-client IOConnection.java
 *
...
class IOConnection {
	/** Debug logger */
	static final Logger logger = Logger.getLogger("io.io.socket");
	...
	/** Socket.io path. */
	public static final String SOCKET_IO_1 = "/io.socket.io/1/";
	...
	/**
	 * The first io.socket to be connected. the io.socket.io server does not send a
	...
	private class HearbeatTimeoutTask extends TimerTask {
		...
		public void run() {
			...
			error(new SocketIOException(
					"Timeout Error. No heartbeat from server within life time of the io.socket. closing.",
					lastException));
		}
	}
	...
	/**
	 * Connects a io.socket to the IOConnection.
	 * 
	 * @param socket
	 *            the io.socket to be connected
	...
	/**
	 * Disconnect a io.socket from the IOConnection. Shuts down this IOConnection
	 * if no further connections are available for this IOConnection.
	 * 
	 * @param socket
	 *            the io.socket to be shut down
	 */
	...
	private IOCallback findCallback(IOMessage message) {
		...
		if (socket == null) {
			logger.warning("Cannot find io.socket for '" + message.getEndpoint() + "'");
